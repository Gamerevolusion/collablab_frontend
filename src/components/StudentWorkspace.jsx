...
import { LANGUAGES, LANG_TEMPLATES, getMonacoLang, getDefaultFileName } from '../utils/languages';
import OutputRenderer from './OutputRenderer';

// ── Module-level constants (avoid recreation on each render) ────
...
// ── IntelliSense keyword/builtin data ───────────────────────────
...
const CPP_KEYWORDS = [...C_KEYWORDS, 'bool', 'catch', 'class', 'const_cast', 'delete',
...
// ── HTML preview builder (shared between editor change and run) ──
...
function makeCompletionProvider(keywords, builtins) {
  return {
...
export default function StudentWorkspace({
  ...
  const [files, setFiles] = useState(() => {
    const defaultName = getDefaultFileName(selectedLanguage);
  ...
  const handleEditorChange = useCallback((value) => {
    ...
    if (isWebMode) {
      setHtmlPreview(buildHtmlPreview(updated));
    }
  ...
  const handleEditorMount = (editor, monaco) => {
    ...
    if (!completionProvidersRegistered.current) {
      ...
      monaco.languages.registerCompletionItemProvider('python', makeCompletionProvider(PYTHON_KEYWORDS, PYTHON_BUILTINS));
      monaco.languages.registerCompletionItemProvider('java', makeCompletionProvider(JAVA_KEYWORDS, JAVA_BUILTINS));
      monaco.languages.registerCompletionItemProvider('c', makeCompletionProvider(C_KEYWORDS, C_BUILTINS));
      monaco.languages.registerCompletionItemProvider('cpp', makeCompletionProvider(CPP_KEYWORDS, [...C_BUILTINS, ...CPP_EXTRAS]));
    }
  };

  const handleRun = () => {
    if (isWebMode) {
      setHtmlPreview(buildHtmlPreview(files));
      setShowPreview(true);
      return;
    }
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            onChange={(e) => {
              ...
              const defaultName = getDefaultFileName(newLang);
        ...
        <div className={`flex-1 w-full flex border-t min-h-0 ${borderClass}`}>
          ...
          <div className={`w-2/3 p-4 overflow-y-auto min-h-0 ${isDark ? 'bg-black' : 'bg-neutral-100'}`}>
            ...
            <div className={`text-xs whitespace-pre-wrap ${isDark ? 'text-neutral-300' : 'text-neutral-700 font-semibold'}`}>
              <OutputRenderer output={terminalOutput} isDark={isDark} />
            </div>
          </div>
        </div>
      )}
